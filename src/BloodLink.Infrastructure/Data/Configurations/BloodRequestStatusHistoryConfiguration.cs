...
public sealed class BloodRequestStatusHistoryConfiguration : IEntityTypeConfiguration<BloodRequestStatusHistory>
{
    public void Configure(EntityTypeBuilder<BloodRequestStatusHistory> builder)
    {
        ...
        builder.HasIndex(history => new { history.BloodRequestId, history.ChangedAtUtc });
        builder.HasOne<BloodRequest>().WithMany().HasForeignKey(history => history.BloodRequestId).OnDelete(DeleteBehavior.NoAction);
